...
export function DocumentationPanel({ repository }: DocumentationPanelProps) {
  ...
  return (
    <div className="h-full flex bg-[#191022]">
      <div className="w-52 flex-shrink-0 border-r border-slate-700/50 bg-slate-900/60 flex flex-col">
        ...
        <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
          {sections.map(s => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs transition-all text-left ${activeSection === s.id
            ...
            >
              ...
              {s.label}
              {activeSection === s.id && <ChevronRight className="w-3 h-3 ml-auto opacity-60" />}
            </button>
          ))}
        </nav>
        ...
        <div className="p-3 border-t border-slate-700/50 space-y-2">
          ...
          <p className="text-[10px] text-slate-600 text-center font-mono">
            {docs.generatedAt.toLocaleTimeString()}
          </p>
        </div>
      </div>
        ...
        <div className="max-w-3xl mx-auto p-8">
          <div className="mb-8 pb-6 border-b border-slate-700/50">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white font-mono">{repository.displayName}</h1>
                <p className="text-xs text-slate-500 font-mono">{repository.fullName}</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 mt-3">
              {repository.language && (
                <span className="px-2.5 py-1 bg-slate-800 border border-slate-700 rounded-md text-xs text-slate-300 font-mono">
                  {repository.language}
                </span>
              )}
              <span className="px-2.5 py-1 bg-slate-800 border border-slate-700 rounded-md text-xs text-slate-500 font-mono">
                ★ {repository.stars}
              </span>
              <span className="px-2.5 py-1 bg-emerald-600/10 border border-emerald-500/20 rounded-md text-xs text-emerald-400 font-mono flex items-center gap-1">
